...
macro_rules! match_tokens{
    ($a:tt+$b:tt)=>{"get an addition"};
    (($i:ident))=>{"get an identifier"};
    ($($other:tt)*)=>{"get something else"};
}
...
macro_rules! what_is1{
    (self)=>{"the keyword `self`"}; //self是关键字，但是走这个
    ($i:ident)=>{concat!("the identifier `",stringify!($i), "`")};
}

macro_rules! call_with_ident{
    ($c:ident($i:ident))=>{$c!($i)}; //这里又把self匹配成关键字了，诡异
}
...
pub fn test1() {
    ...
    println!("{}\n{}\n{}\n",match_tokens!((caravan)), match_tokens!(3+6),match_tokens!(5));
    // get an identifier
    // get an addition
    // get something else
    println!("{}\n{}\n{}",capture_then_match_tokens!((caravan)),capture_then_match_tokens!(3+6),
             capture_then_match_tokens!(5));
    // get something else
    // get something else
    // get something else
    println!("{}\n{}\n{}\n{}", what_is!(#[no_mangle]),what_is!(#[inline]),
             capture_then_what_is!(#[no_mangle]),capture_then_what_is!(#[inline]),);
    // no_mangle attribute
    // inline attribute

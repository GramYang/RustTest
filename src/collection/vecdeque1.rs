...
//模仿go的sync.Pool封装一个Pool
pub fn vd2(){
    let p = Arc::new(Pool::new(100));
    for _ in 1..101{
        let p1 = Arc::clone(&p);
        thread::spawn(move ||{
            p1.set(Bag::default())
    ...
    println!("{}",p.len());
    let p1 = Pool::<Bag>::new(100);
    println!("{:?}",p1.get());
}

struct Pool<T>(RwLock<Inner<T>>);
...
impl<T:Default> Pool<T>{
    fn new(c:usize) -> Pool<T> {
        Pool{
            0:RwLock::new(Inner{buf:VecDeque::<T>::new(),cap:c}),
        }
    }
    ...
    fn set(&self,t:T) ->Result<(),String>{
        let mut a  = self.0.try_write();
        match a {
            Ok(mut x) => {
                x.buf.push_front(t);
                Ok(())
